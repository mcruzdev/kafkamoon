...
public class CreateTopicUseCase {
   ...
   public CreateTopicUseCaseOutput execute(final CreateTopicUseCaseInput input) {
      ...
      TopicCriticality criticality = TopicCriticality.fromString(input.criticality().toUpperCase(Locale.ROOT));

      LOGGER.info("[flow:create.topic][criticality:{}] Create topic with name {}, partitions {} and"
                  + " replication factor {}", criticality.name(), topicName, criticality.getPartitions(),
            criticality.getReplicationFactor());

      String topicId = this.kafkaClient.createTopic(topicName.finalName(), criticality.getPartitions(),
            criticality.getReplicationFactor());
      LOGGER.info("[flow:create.topic][status:success] Topic with name '{}' has the id '{}'", topicName, topicId);
