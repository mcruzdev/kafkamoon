...
public class DefaultKafkaClient implements KafkaClient {
   ...
   public KafkaNodeDetails getClusterInfo() {
      try (AdminClient adminClient = AdminClient.create(this.props)) {
         ...
         return new KafkaNodeDetails(kafkaNode.idString(), kafkaNode.host(), kafkaNode.hasRack(), kafkaNode.rack(),
               kafkaNode.port(), kafkaNode.isEmpty(), null);
      } catch (ExecutionException | InterruptedException e) {
   ...
   public Optional<TopicDescription> getTopicByName(String name) {
      try (AdminClient adminClient = AdminClient.create(this.props)) {
         Map<Uuid, TopicDescription> response = adminClient.describeTopics(TopicCollection.ofTopicIds(List.of(
